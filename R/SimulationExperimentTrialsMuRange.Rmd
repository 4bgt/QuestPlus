---
title: "Simulation: Constant Stimuli vs Quest+ with borader Mu range and varying number of trials"
author: "Niklas Stein"
date: "`r Sys.Date()`"
...
library(ggplot2)
library(quickpsy)
for (repeatedTrials in c(6))
{
  #### Constant Stimuli ####
  constantStimuliTest <- read_csv(paste0("constantStimuliMu",repeatedTrials,".csv"),show_col_types = FALSE)
  ...
  for (mu in unique(constantStimuliTest$trueMu))
  {
    for (sigma in c(0.1))
    {
      for (saturation in unique(constantStimuliTest$trueSaturation))
      {
        ...
        constantPlot=plot(fit)+geom_point(data = average_responses,aes(stimulus,Mean,color=simulationRun,group = simulationRun))+geom_vline(xintercept=mu)+xlim(-0.21,0.21)
  ...
  ##### Quest+ #####
  QuestPlusTest <- read_csv(paste0("Quest+Mu",repeatedTrials,".csv"))
