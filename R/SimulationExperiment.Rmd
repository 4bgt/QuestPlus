...
```{r constant stimuli, echo=FALSE,message=F,warning=FALSE}
...
for (repeatedTrials in c(1:10))
{
  ...
  {
    for (sigma in unique(constantStimuliTest$trueSigma))
    {
...
```{r quest+, echo=FALSE,message=F,warning=FALSE}
knitr::opts_chunk$set(echo = TRUE)
...
for (repeatedTrials in c(1:10))
{
  ...
  {
    for (sigma in unique(QuestPlusTest$trueSigma))
    {
